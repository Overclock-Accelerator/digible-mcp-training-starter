"""Crossword agent — one of three consumers of the shared PuzzleMaster server.

    python agent_crossword.py --pattern C_O__W_RD

Structurally identical to agent_bee.py and agent_wordle.py. The only
...
MY_TOOL = "solve_crossword_pattern"
DEFAULT_AGENT_NAME = "agent-crossword"


async def main() -> int:
    parser = argparse.ArgumentParser(description="Solve a crossword pattern via MCP.")
    parser.add_argument("--server", default="http://127.0.0.1:8002/mcp",
                        help="URL of the running MCP server")
    parser.add_argument("--pattern", help="letters with _ for unknowns, e.g. C_O__W_RD")
    ...
    async with client.session("puzzlemaster") as session:
        ...
        agent = create_agent(
            ...
            tools=tools,
            system_prompt=(
                "You are a crossword assistant. Always use the tool to find matches; "
                f"never guess words yourself. Pass agent_name=\"{args.agent_name}\" on "
                "every tool call. Report how many words matched and list them."
            ),
        )

        # ainvoke, not invoke: MCP tools arrive as coroutine-only StructuredTools.
        if getattr(args, "question", None) or repl.one_shot(args, 'pattern'):
            question = getattr(args, "question", None) or f"Which words fit the crossword pattern {args.pattern}?"
            print(f"\nyou › {question}")
            print(await repl.once(agent, question))
        else:
            await repl.chat(
                    agent,
                    title='Crossword agent — shared PuzzleMaster server',
                    hints=[
                    'try:  what fits C_O__W_RD?',
                    'then: how about _ANGUAGE',
                    ],
                )
    return 0
